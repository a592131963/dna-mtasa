...
function shopBuyInvisibleCommand (thePlayer, command)
  if not (isGuestAccount (getPlayerAccount (thePlayer))) then
    if (getAccountData (getPlayerAccount (thePlayer), "funmodev2-korting")) then
      if (isElementWithinMarker (thePlayer, invisibleShopMarker)) and (getPlayerMoney (thePlayer) >= 100) then
        ...
        outputChatBox ("This costs you 100$. (you've got a cart!)", thePlayer, 255, 0, 0, getRootElement())
      else
        outputChatBox ("You must be in the magicshop to get invisible or you are too poor (100$).", thePlayer, 255, 0, 0, getRootElement())
      end
    else
