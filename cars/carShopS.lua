...
function(id, cost, name)
	if (getPlayerMoney (source) >= tonumber(cost)) then
		...
		setAccountData (getPlayerAccount (source), "rpg-car", tonumber(id))
		setAccountData (getPlayerAccount (source), "rpg-paintjob", 3)
		setAccountData (getPlayerAccount (source), "rpg-carUpgrades", 0)
	else
